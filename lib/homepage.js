...
const { decodeHtmlEntities, stripHtml } = require('./security');
...
function cleanText(value, maxLength = 4000) {
  return decodeHtmlEntities(stripHtml(value || '')).replace(/\s+/g, ' ').trim().slice(0, maxLength);
}

function decodeContentEntities(value) {
  if (typeof value === 'string') {
    return decodeHtmlEntities(value);
  }

  if (Array.isArray(value)) {
    return value.map((item) => decodeContentEntities(item));
  }

  if (value && typeof value === 'object') {
    return Object.fromEntries(
      Object.entries(value).map(([key, nestedValue]) => [key, decodeContentEntities(nestedValue)])
    );
  }
...
function parseStoredContent(payloadJson) {
  ...
  try {
    return decodeContentEntities(mergeHomepageContent(JSON.parse(payloadJson)));
  } catch (error) {
    return cloneDefaults();
  }
}
...
function normalizeHomepageInput(input) {
  ...
  const content = {
    ...
    testimonials: {
      enabled: input.testimonials_enabled === 'on' || input.testimonials_enabled === 'true' || input.testimonials_enabled === true,
  ...
  if (content.testimonials.enabled && !content.testimonials.title) errors.push('Testimonials section title is required when the section is enabled.');
  if (content.testimonials.enabled && !content.testimonials.intro) errors.push('Testimonials intro is required when the section is enabled.');
  if (content.testimonials.enabled && content.testimonials.items.some((item) => !item.text || !item.name || !item.role)) errors.push('All six testimonials require text, client name, and role when the section is enabled.');
